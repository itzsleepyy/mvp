...
//! instance and exits. Used by Claude Code hook commands, which must never
...
/// Sends `event` to the running instance discovered via the default socket
/// file. Errors mean "WaitState is not running" and are safe to ignore.
pub fn send_event(event: AgentEvent) -> Result<(), String> {
    send_event_to(socket_path(), event)
}

/// Same as [`send_event`], with an explicit socket path (used by tests).
pub fn send_event_to(path: PathBuf, event: AgentEvent) -> Result<(), String> {
    ...
    let message = AgentMessage::new(&info.token, event);
...
mod tests {
    ...
    fn send_event_to_missing_socket_file_fails_cleanly() {
        ...
        let result = send_event_to(path, AgentEvent::Working);
